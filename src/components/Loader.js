...
const Loader = () => {
  ...
  return (
    <div className="loader-container">
      ...
      {/* TrueFocus анімація для "click here" */}
      <div className="click-focus-container" ref={containerRef}>
        {words.map((word, index) => (
          <motion.span
            key={index}
            ref={(el) => (wordRefs.current[index] = el)}
            className={`click-focus-word ${currentIndex === index ? 'active' : ''}`}
            animate={{
              filter: currentIndex === index ? 'blur(0px)' : 'blur(3px)',
              scale: currentIndex === index ? 1.1 : 1,
              textShadow: currentIndex === index 
                ? '0 0 25px #91DDFB, 0 0 50px #5927CE, 0 0 75px #91DDFB'
                : '0 0 10px rgba(145, 221, 251, 0.3)'
            }}
        ...
        <motion.div
          ...
          transition={{ duration: 0.6, ease: 'easeInOut' }}
        >
          <div className="click-corner top-left"></div>
          <div className="click-corner top-right"></div>
          <div className="click-corner bottom-left"></div>
          <div className="click-corner bottom-right"></div>
        </motion.div>
      </div>
    </div>
  );
